# 📘 **README — GWAS QC + GCTA MLM Pipeline**
...
project/
│
├── gwas_qc_pipeline_improve.sh       # Full QC workflow (PLINK)
├── gwas_qc_gcta.pbs                  # PBS script (QC → GCTA → plots)
├── gwas_gcta_plots.R                 # Manhattan/QQ/λGC generator
│
├── qc_results/
│   ├── step1_initial/
│   ├── step2_snp_qc/
│   ├── step3_sample_qc/
│   ├── step4_relatedness/
│   ├── step5_final/                 # QC final PLINK files
│   ├── gcta/                        # GCTA results (GRM + MLMA)
│   └── plots/                       # Figures (PCA, QQ, Manhattan)
│
└── logs/                            # HPC job logs
```
...
| GCTA | ≥1.94 | Kinship matrix + Mixed-model association |
| R | ≥4.0 | Manhattan/QQ plots |
| PBS / SLURM | — | Job scheduling on HPC |

---

# 🚀 **2. Launch the Full Pipeline (QC → GCTA → Plots)**
...
# 🔍 **3. GWAS QC (gwas_qc_pipeline_improve.sh)**
...
# 🧩 **4. Kinship Matrix (GRM) with GCTA**
...
```
lambda_gc = median(χ²_observed) / median(χ²_expected)
```
...
### **QQ Plot**
- Points close to diagonal → well-calibrated model  
- Early upward deviation → polygenicity  
- Extreme inflation → population structure not fully corrected  

### **Manhattan Plot**
- Red dashed line = genome-wide significance (5e-8)
- Peaks above threshold → candidate loci  

### **λGC Interpretation**
| λGC | Interpretation |
|------|----------------|
| 1.00–1.05 | Excellent, no inflation |
| 1.10–1.20 | Acceptable for polygenic traits |
